...
Hint: to quick start using the asset just add VolumetricFog script to your camera. You can also right click on the Hierarchy panel and select Create Other -> Volumetric Fog.
...
v11.1
- Fog of War: added "Reuse Fog of War" option which allows fog areas reuse main camera fog script fog of war settings and layout

v11.0
- Added Terrain Fit option to fog areas

v10.9.3
- [Fix] Fixed fog of war blur reset issue

v10.9.2
- Fog of war editor: improved workflow when painting fog of war and there's no geometry under cursor

v10.9.1
- [Fix] Fixed inspector issue not showing shader keywords after switching to local keywords usage

v10.9
- Minimum Unity version is now 2019.4
- Added support for integrations with vertex/fragment shaders
- Reduced global keywords usage
- [Fix] Fixed some code not respecting the ENABLE_XR define

v10.8.6
- Added "ENABLE_XR" macro at the start of VolumetricFog.cs script. Can be commented out to disable usage of XR module.
- Improved reliability of fog of war delayed transitions

v10.8.5
- [Fix] Fixed Fog Area/Void positions in orthographic camera mode

v10.8.4
- Fog of war holes can now work with independent fog areas
- Updated documentation with common properties used at runtime
- Fixed demo scene 2 location folder

v10.8.3
- Fog of war can now be used with void areas at same time

v10.8.2
- Upgraded method to auto-detect VR Single Pass Stereo

v10.8.1
- [Fix] Fixed Multi Camera option getting disabled when changing fog area position in Unity Editor

v10.8
- Added "Show Gizmo" option under Fog Void section

v10.7.1
- [Fix] Fixed VR issue on Quest with MultiView rendering

v10.7
- New shader option: "World Space Noise in Fog Areas". Now fog areas default to use local space when computing noise. Enable this option to revert to previous behaviour.

v10.6.5
- Minor improvements

v10.6.3
- Fixes and improvements regarding multiple fog areas

v10.6.2
- Editor fixes and minor improvements

v10.6.1
- [Fix] Fixed console error when trying to destroy a fog of war mask texture asset

v10.6
- Fog of War: new Blur option (smooth differences between different alpha values across the fog of war)

v10.5
- New advanced option: ray-marched fog of war. Improves quality of fog of war. Video: https://youtu.be/stoV7C_QTIg

v10.4.1
- Sun diffusion now can be completely disabled in Shader Options (which can save some GPU cycles)
- [Fix] Removed tiny GC in Editor (not runtime) when rendering the fog effect in SceneView
- [Fix] Fixed Sun diffusion not enabled when using downsampling

v10.4
- Support for directional light cookie. Enable it in "Shader Options" section.
- Improved fog fade out/blending in the distance

v10.3
- Sun shadows now use Unity's directional shadow map by default. New option in "Shader Options" section to enable custom shadow pass (previous behaviour).

v10.2
- Added visibility scope property to Fog Geometry. Can hide automatically fog areas if rendering camera is outside of a given volume.
- Fog instances now can be listed in Volumetric Fog inspector for debugging purposes
- [Fix] Fixed issue during start up which lead to brief incorrect fog area sorting
- [Fix] Improved default camera selector when no camera is tagged as main camera

v10.1
- Added Enviro Integration: Volumetric Fog Day Cycle Manager script that can be attached to the camera to sync time of day and special colors/densities
- Max Distance can now be increased beyond camera far clip
- Sky Haze depth parameter can now be reduced to produce distance fog
- Added noise scale parameter to Sky haze section
- Added a few recommendation messages to inspector based on certain parameters values
- Dynamic Fog & Mist 6.5.5

v10.0
- Fog of War: can edit fog of war coverage in SceneView interactively using the new Fog Of War Editor inspector options
- Improvements to sky haze. Fog now blends seamlessly with sky haze at distance when sky haze is on
- Improved max distance/max distance threshold effect on fog
- Some shader optimizations & internal improvements
- [Fix] Fixed unimportant console error when Smooth Scattering Rays is enabled with VR Single Pass Instanced

v9.9
- New option "Show In SceneView"
- New Volumetric Fog Light Params component which is automatically attached to tracked point lights to allow per-light range and intensity multiplier
- Added Smooth Scattering Rays option to Shader Options (Unity 2018.3+). Adds separate blur to light scattering effect
- Improved dithering, less artifacting
- [Fix] Fixed Scattering Rays in VR Multi Pass mode
- [Fix] Fixed right eye displacement in VR Single Pass Instanced mode when transparency blend is enabled (Unity 2018.3+)

v9.8 2019-APR-05
- New Fog of War methods and options. Improved fog of war methods execution performance.
- Added contextual option (right click in hierarchy panel -> Create Other -> Volumetric Fog -> Fog Of War Hole)

v9.7 2019-APR-01
- Added height falloff parameter
- Added deep obscurance parameter
- Sky haze improvements
