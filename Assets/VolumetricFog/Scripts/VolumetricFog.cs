...
namespace VolumetricFogAndMist {
    ...
    public partial class VolumetricFog : MonoBehaviour {
        ...
        const string VFM_BUILD_HINT = "VFMBuildHint111RC2";
        ...
        [SerializeField]
        SPSR_BEHAVIOUR _spsrBehaviour = SPSR_BEHAVIOUR.AutoDetect;
        ...
        void LateUpdate() {
            ...
            if (fogRenderer.sun != null) {
                ...
                bool mayUpdateTexture = !Application.isPlaying || (updatingTextureSlice < 0 && now - lastTextureUpdate >= _timeBetweenTextureUpdates);
                if (mayUpdateTexture) {
                ...
                if (targetFogAlpha >= 0 || targetSkyHazeAlpha >= 0) {
                    if (targetFogAlpha != currentFogAlpha || targetSkyHazeAlpha != currentSkyHazeAlpha) {
                        if (transitionDuration > 0) {
                            currentFogAlpha = Mathf.Lerp(initialFogAlpha, targetFogAlpha, (now - transitionStartTime) / transitionDuration);
                            currentSkyHazeAlpha = Mathf.Lerp(initialSkyHazeAlpha, targetSkyHazeAlpha, (now - transitionStartTime) / transitionDuration);
                        } else {
                    ...
                    if (transitionDuration > 0) {
                        currentFogAlpha = Mathf.Lerp(initialFogAlpha, _alpha, (now - transitionStartTime) / transitionDuration);
                        currentSkyHazeAlpha = Mathf.Lerp(initialSkyHazeAlpha, alpha, (now - transitionStartTime) / transitionDuration);
                    } else {
                        currentFogAlpha = _alpha;
                        currentSkyHazeAlpha = _skyAlpha;
                        transitionAlpha = false;
            ...
            if (transitionColor) {
                if (targetColorActive) {
                    if (targetFogColor != currentFogColor) {
                        if (transitionDuration > 0) {
                            currentFogColor = Color.Lerp(initialFogColor, targetFogColor, (now - transitionStartTime) / transitionDuration);
                        } else {
                    ...
                    if (transitionDuration > 0) {
                        currentFogColor = Color.Lerp(initialFogColor, _color, (now - transitionStartTime) / transitionDuration);
                    } else {
                        currentFogColor = _color;
                        transitionColor = false;
                    }
            ...
            if (transitionSpecularColor) {
                if (targetSpecularColorActive) {
                    if (targetFogSpecularColor != currentFogSpecularColor) {
                        if (transitionDuration > 0) {
                            currentFogSpecularColor = Color.Lerp(initialFogSpecularColor, targetFogSpecularColor, (now - transitionStartTime) / transitionDuration);
                        } else {
                    ...
                    if (transitionDuration > 0) {
                        currentFogSpecularColor = Color.Lerp(initialFogSpecularColor, _specularColor, (now - transitionStartTime) / transitionDuration);
                    } else {
                        currentFogSpecularColor = _specularColor;
                        transitionSpecularColor = false;
                    }
            ...
            if (transitionLightColor) {
                if (targetLightColorActive) {
                    if (targetLightColor != currentLightColor) {
                        if (transitionDuration > 0) {
                            currentLightColor = Color.Lerp(initialLightColor, targetLightColor, (now - transitionStartTime) / transitionDuration);
                        } else {
                    ...
                    if (transitionDuration > 0) {
                        currentLightColor = Color.Lerp(initialLightColor, _lightColor, (now - transitionStartTime) / transitionDuration);
                    } else {
                        currentLightColor = _lightColor;
                        transitionLightColor = false;
                    }
...
#if UNITY_EDITOR
                    useSinglePassStereoRenderingMatrix = XRSettings.stereoRenderingMode != XRSettings.StereoRenderingMode.MultiPass || PlayerSettings.stereoRenderingPath != StereoRenderingPath.MultiPass;
#else
...
#pragma warning restore 0162
            ...
            if (vrOn && fogRenderer.useSinglePassStereoRenderingMatrix) {
                ...
                fogMat.SetMatrix(ShaderParams.ClipToWorld, mainCamera.cameraToWorldMatrix * mainCamera.projectionMatrix.inverse);
            }
            ...
            if (_downsampling > 1f || _forceComposition) {
                ...
                rtReducedDestinationDesc.colorFormat = rtDownsampledFormat;
                RenderTexture reducedDepthTexture = RenderTexture.GetTemporary(rtReducedDestinationDesc);
...
#region Settings area
        ...
        void UpdateMaterialHeights(Camera mainCamera) {
            ...
            fogMat.SetVector(ShaderParams.FogData, new Vector4(currentFogAltitude, h, 1.0f / (_density * temporaryProperties.density), scale));
        ...
        void UpdateMaterialPropertiesNow(Camera mainCamera) {
            ...
            fogMat.SetVector(ShaderParams.FogDistance, new Vector4(scale * scale * _distance * _distance, (_distanceFallOff * _distanceFallOff + 0.1f), _maxFogLength, maxFogLengthFallOff));
            UpdateMaterialFogColor();
        ...
        void UpdateSunShadowsData() {
            ...
            if (_hasCamera) {
                Shader.SetGlobalVector(ShaderParams.GlobalSunShadowsData, new Vector4(shadowStrength, _sunShadowsJitterStrength, _sunShadowsCancellation, 0));
#pragma warning disable 0162
#pragma warning disable 0429
...
#pragma warning restore 0162
        ...
        void UpdateWindSpeedQuick() {
            ...
            Vector4 skySpeedData = new Vector4(_skyHaze, _skyNoiseStrength / (0.0001f + _density * temporaryProperties.density), skyHazeSpeedAcum, _skyDepth);
...
#if ENABLE_XR
            bool usingXR = UnityEngine.XR.XRSettings.enabled;
#else
            bool usingXR = false;
#endif

            Vector3 viewportPos = mainCamera.WorldToViewportPoint(sunSkyPos, usingXR ? Camera.MonoOrStereoscopicEye.Left : Camera.MonoOrStereoscopicEye.Mono);
            if (viewportPos.z < 0) {
                if (isOrtho) {
                ...
                fogMat.SetVector(ShaderParams.FogScatteringData, new Vector4(_lightScatteringSpread / _lightScatteringSamples, intensity > 0 ? _lightScatteringSamples : 0, intensity, _lightScatteringWeight / (float)_lightScatteringSamples));
                fogMat.SetVector(ShaderParams.FogScatteringData2, new Vector4(_lightScatteringIllumination, _lightScatteringDecay, _lightScatteringJittering, _lightScatteringEnabled && LIGHT_DIFFUSION_ENABLED ? 1.2f * _lightScatteringDiffusion * night * sunLightIntensity : 0));
