...
namespace VolumetricFogAndMist {
    ...
    public class VolumetricFogInspector : Editor {
        ...
        SerializedProperty _fogOfWarEnabled, _fogOfWarReuseOther, _fogOfWarOther, _fogOfWarTexture, _fogOfWarCenter, _fogOfWarSize, _fogOfWarTextureSize, _fogOfWarRestoreDelay, _fogOfWarRestoreDuration, _fogOfWarSmoothness, _fogOfWarBlur;
        SerializedProperty _fogVoidPosition, _character, _fogVoidTopology, _fogVoidRadius, _fogVoidFallOff, _fogVoidHeight, _fogVoidDepth, _fogVoidShowGizmos;
        ...
        void OnEnable() {
            ...
            _fogOfWarReuseOther = serializedObject.FindProperty("_fogOfWarReuseOther");
            _fogOfWarOther = serializedObject.FindProperty("_fogOfWarOther");
            ...
            _fogVoidShowGizmos = serializedObject.FindProperty("_fogVoidShowGizmos");
            ...
            terrainFit = serializedObject.FindProperty("_terrainFit");
            terrainFitResolution = serializedObject.FindProperty("_terrainFitResolution");
            terrainLayerMask = serializedObject.FindProperty("_terrainLayerMask");
            terrainFogHeight = serializedObject.FindProperty("_terrainFogHeight");
            terrainFogMinAltitude = serializedObject.FindProperty("_terrainFogMinAltitude");
            terrainFogMaxAltitude = serializedObject.FindProperty("_terrainFogMaxAltitude");
        ...
        public override void OnInspectorGUI() {
            ...
            if (expandFogGeometrySection) {
                ...
                EditorGUI.indentLevel--;
                if (_fog.isTerrainFitActive) {
                    EditorGUILayout.HelpBox("Height is controlled by Terrain Fit's 'Height Over Surface' parameter.", MessageType.Info);
                    GUI.enabled = false;
                }
...
#pragma warning restore 0429
            ...
            if (IsFeatureEnabled(VolumetricFog.SKW_FOG_OF_WAR_ON)) {
                ...
                if (expandFogOfWarSection) {
                    ...
                    if (_fogOfWarEnabled.boolValue && !_fogOfWarReuseOther.boolValue) {
                        if (GUILayout.Button("Reset", GUILayout.Width(60)))
                            _fog.ResetFogOfWar();
                    }
                    EditorGUILayout.EndHorizontal();

                    if (_fogOfWarEnabled.boolValue) {
                        EditorGUILayout.PropertyField(_fogOfWarReuseOther, new GUIContent("Reuse Fog Of War", "Use fog of war settings and contents from other fog component."));
                        if (_fogOfWarReuseOther.boolValue) {
                            EditorGUI.indentLevel++;
                            EditorGUILayout.PropertyField(_fogOfWarOther, new GUIContent("Fog Of War", "Assign the fog component that handles fog of war settings and contents."));
                            EditorGUI.indentLevel--;
                            EditorGUILayout.HelpBox("Customize fog of war from the fog component of the referenced component.", MessageType.Info);
                            if (GUILayout.Button("Select Fog Of War Holder")) {
                                Selection.activeGameObject = _fog.fogOfWarOther.gameObject;
                                GUIUtility.ExitGUI();
                            }
                        }
                    }
            ...
            if (!fogVoidBoxEnabled && !fogVoidSphereEnabled) {
                ...
                if (expandFogVoidSection) {
                    ...
                    EditorGUILayout.PropertyField(_character, new GUIContent("Object To Follow", "Assign a Game Object to follow its position automatically (usually the player character which will be at the center of the void area)."));
                    ...
                    EditorGUILayout.PropertyField(_fogVoidShowGizmos, new GUIContent("Show Gizmo", "Shows a bounding box around the void volume in SceneView."));
                }
            }
            EditorGUILayout.EndVertical();
            EditorGUILayout.Separator();
            ...
            if (!fogAreaBoxEnabled && !fogAreaSphereEnabled) {
                ...
                expandFogAreaSection = EditorGUILayout.Foldout(expandFogAreaSection, _fog.isFogAreaActive ? FOG_AREA_ON : FOG_AREA_OFF, sectionHeaderStyle);
                ...
                if (expandFogAreaSection) {
                    ...
                    bool terrainFitEnabledInShader = IsFeatureEnabled(VolumetricFog.SKW_SURFACE);
                    if (terrainFitEnabledInShader) {
                        EditorGUILayout.PropertyField(terrainFit, new GUIContent("Terrain Fit", "Adjust fog elevation to the terrain surface."));
                        if (terrainFit.boolValue) {
                            if (_fog.isFogAreaActive) {
                                EditorGUI.indentLevel++;
                                EditorGUILayout.PropertyField(terrainLayerMask, new GUIContent("Layer Mask", "Layer mask for the terrain."));
                                EditorGUILayout.PropertyField(terrainFitResolution, new GUIContent("Resolution"));
                                EditorGUILayout.PropertyField(terrainFogHeight, new GUIContent("Height Over Surface", "Height of the fog above the terrain surface."));
                                ...
                                EditorGUILayout.HelpBox("Terrain Fit option requires use Fog of Area. Increase the radius to start creating a fog area.", MessageType.Info);
                            }
                        }
                    } else {
                        EditorGUILayout.BeginHorizontal();
                        GUILayout.Label("Terrain Fit", GUILayout.Width(EditorGUIUtility.labelWidth));
                        DrawFeatureIsDisabled();
                        EditorGUILayout.EndHorizontal();
                    }
        ...
        void CreateProfile() {
            ...
            VolumetricFogProfile newProfile = CreateInstance<VolumetricFogProfile>();
        ...
        private void OnSceneGUI() {
            ...
            if (hit) {
                ...
                Bounds bounds = _fog.GetBounds();
                bounds = new Bounds(bounds.center, new Vector3(bounds.size.x, 0.01f, bounds.size.z));
                float distance;
            ...
            if (hit) {
                ...
                if (e.isMouse && e.button == 0) {
                    ...
                    if (eventType == EventType.MouseDown) {
                        ...
                        if (floating) {
                            Debug.LogWarning("No geometry found under cursor while painting fog of war. Consider enabling the 'Raymarched Fog of War' shader option.");
                        }
